...
export default function Compteur({ stats }) {
  ...
  const { value, unit } = formatCounter(total, intl, langue);

  const lignes = [
    ['compteur.enChemin', stats?.enTransit ?? 0, 'text-sauge-fonce'],
    ['compteur.arrives', stats?.arrives ?? 0, 'text-corail-fonce'],
    ['compteur.hibernation', stats?.hibernants ?? 0, 'text-pluie-fonce'],
    ['compteur.perdus', stats?.perdus ?? 0, 'text-encre-pale'],
  ];
  ...
  return (
    <section className="feuille relative overflow-hidden rounded-petale px-8 py-9">
      ...
      <p className="etiquette">{t('compteur.depuis')}</p>
